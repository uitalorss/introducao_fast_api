...
async def put_arigo(artigo_id: int, artigo: ArtigoSchema, db: AsyncSession = Depends(get_session), usuario_logado: UsuarioModel = Depends(obter_usuario_atual)):
     async with db as session:
        query = select(ArtigoModel).filter(ArtigoModel.id == artigo_id)
        result = await session.execute(query)
        ...
        if artigo.url_fonte:
            update_artigo.url_fonte = artigo.url_fonte
...
async def delete_artigo(artigo_id: int, usuario_logado: UsuarioModel = Depends(obter_usuario_atual), db: AsyncSession = Depends(get_session)):
    async with db as session:
        query = select(ArtigoModel).filter(ArtigoModel.id == artigo_id)
        result = await session.execute(query)
        delete_artigo: ArtigoModel = result.scalar_one_or_none()
